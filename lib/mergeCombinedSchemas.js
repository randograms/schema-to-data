...
const MERGE_RULES = {
  ...
  object: {
    ...
    patternProperties: {
      ...
      pickByValue: (patternProperties1, patternProperties2) => {
        const sharedPropertyPatterns = _.intersection(_.keys(patternProperties1), _.keys(patternProperties2));
        const combinedSharedProperties = (
          _(sharedPropertyPatterns)
            .map((propertyName) => [
              propertyName,
              pickSchema(patternProperties1[propertyName], patternProperties2[propertyName]),
            ])
            .fromPairs()
            .value()
        );
