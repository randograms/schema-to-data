...
const generateString = function (stringSchema) {
  ...
  if (pattern !== null) {
    let generatedString;
    try {
      generatedString = this.generateStringFromPattern(pattern);
    } catch (error) {
      if (!this.OPTIONS.onUnknownPattern) {
        // eslint-disable-next-line max-len
        throw Error(`Failed to generate data for regex pattern "${pattern}" due to: "${error.message}". If the pattern is valid, then consult the README for the "onUnknownPattern" option.`);
      }

      generatedString = this.OPTIONS.onUnknownPattern(stringSchema);
    }
